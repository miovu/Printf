...
int	ft_printf(const char *s, ...)
{
	...
	{
		if (s[finder] != '%')
			counter += ft_putchar_fd(s[finder++], 1);
		else if (s[finder] == '%')
		{
			finder++;
			if (s[finder] == ' ')
...
int	printf_format(const char *format, va_list args)
{
	...
	print = 0;
	if (*format == 'c')
		print += ft_putchar_fd(va_arg(args, int), 1);
	else if (*format == 's')
		print += ft_putstr_fd(va_arg(args, char *), 1);
	else if (*format == 'p')
		print += ft_putptr_fd(va_arg(args, void *), 1);
	if (*format == 'd' || *format == 'i')
		print += ft_putnbr_fd(va_arg(args, int), 1);
	else if (*format == 'u')
		print += ft_putnbr_un_fd(va_arg(args, unsigned int), 1);
	else if (*format == 'x')
